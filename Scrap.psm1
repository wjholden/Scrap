...
function Get-NetworkAudit {
    ...
    process {
        # Using low-tech jobs instead of ForEach-Object -Parallel for compatibility
        # with PowerShell before PS7.
        $jobs = $ComputerName | ForEach-Object {
            Start-Job -ArgumentList @($_, $Username, $commands, $rules) -ScriptBlock {
                param($computer, $Username, $commands, $rules)
                $queries = @{};
        ...
        $jobs | Wait-Job | Receive-Job | Select-Object -ExcludeProperty RunspaceId
    }
}
